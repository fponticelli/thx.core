...
class Dynamics {
	...
	public static function parseNonEmptyString0<E>(v:Dynamic, err:String->E):VNel<E, String>
		return parseString0(v, err).flatMapV(function(str:String) return if (str == null || str.length == 0) {
	...
	public static function parseNel<E, A>(v:Dynamic, f:Dynamic->VNel<E, A>, err:String->E):VNel<E, Nel<A>>
		return parseArray(v, f, err).flatMapV(function(xs:Array<A>) {
	...
	public static function parseStringMap<E, K, V>(v:Dynamic, f:Dynamic->String->VNel<E, V>, err:String->E):VNel<E, std.Map<String, V>> {
		return if (Types.isAnonymousObject(v)) {
			Reflect.fields(v)
				.traverseValidation(function(field:String) return f(Reflect.getProperty(v, field), field).map(Tuple.of.bind(field, _)), Nel.semigroup())
	...
	public static function parseTuple3<Err, A, B, C>(v:Dynamic, fa:Dynamic->VNel<Err, A>, fb:Dynamic->VNel<Err, B>, fc:Dynamic->VNel<Err, C>,
			err:String->Err):VNel<Err, thx.Tuple3<A, B, C>> {
		return if (Types.isAnonymousObject(v)) {
	...
	public static function parseTuple4<Err, A, B, C, D>(v:Dynamic, fa:Dynamic->VNel<Err, A>, fb:Dynamic->VNel<Err, B>, fc:Dynamic->VNel<Err, C>,
			fd:Dynamic->VNel<Err, D>, err:String->Err):VNel<Err, thx.Tuple4<A, B, C, D>> {
		return if (Types.isAnonymousObject(v)) {
			val4(Tuple4.of, parseProperty(v, "_0", fa, err), parseProperty(v, "_1", fb, err), parseProperty(v, "_2", fc, err),
				parseProperty(v, "_3", fd, err), Nel.semigroup());
		} else {
			failureNel(err('$v is not a tuple4-object (type resolved to ${Type.typeof(v)})'));
		};
	}

	public static function parseTuple5<Err, A, B, C, D, E>(v:Dynamic, fa:Dynamic->VNel<Err, A>, fb:Dynamic->VNel<Err, B>, fc:Dynamic->VNel<Err, C>,
			fd:Dynamic->VNel<Err, D>, fe:Dynamic->VNel<Err, E>, err:String->Err):VNel<Err, thx.Tuple5<A, B, C, D, E>> {
		return if (Types.isAnonymousObject(v)) {
			val5(Tuple5.of, parseProperty(v, "_0", fa, err), parseProperty(v, "_1", fb, err), parseProperty(v, "_2", fc, err),
				parseProperty(v, "_3", fd, err), parseProperty(v, "_4", fe, err), Nel.semigroup());
		} else {
			failureNel(err('$v is not a tuple5-object (type resolved to ${Type.typeof(v)})'));
		};
	}

	public static function parseTuple6<Err, A, B, C, D, E, F>(v:Dynamic, fa:Dynamic->VNel<Err, A>, fb:Dynamic->VNel<Err, B>, fc:Dynamic->VNel<Err, C>,
			fd:Dynamic->VNel<Err, D>, fe:Dynamic->VNel<Err, E>, ff:Dynamic->VNel<Err, F>, err:String->Err):VNel<Err, thx.Tuple6<A, B, C, D, E, F>> {
		return if (Types.isAnonymousObject(v)) {
			val6(Tuple6.of, parseProperty(v, "_0", fa, err), parseProperty(v, "_1", fb, err), parseProperty(v, "_2", fc, err),
				parseProperty(v, "_3", fd, err), parseProperty(v, "_4", fe, err), parseProperty(v, "_5", ff, err), Nel.semigroup());
		} else {

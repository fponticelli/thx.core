	...
	public function equalsWith(that:KTree<T>, fn:T->T->Bool):Bool {
		function handler(ls0:Null<List<KTree<T>>>, ls1:Null<List<KTree<T>>>):Bool {
			return switch ([ls0, ls1]) {
				case [Cons(x, xs), Cons(y, ys)]: var l:KTree<T> = x; var r:KTree<T> = y; l.equals(r) && handler(xs, ys);
				case [Nil, Nil]: true;
				case [null, null]: true;
				default: false;
			}
...
class KTrees {
	 ...
	 */
	static public function iterDF<T>(node:KTree<T>):Iterable<T>
		return iter(genDF(node));
	 ...
	 */
	static public function iterBF<T>(node:KTree<T>):Iterable<T>
		return iter(genBF(node));

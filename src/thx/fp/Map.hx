...
abstract Map<K, V>(MapImpl<K, V>) from MapImpl<K, V> to MapImpl<K, V> {
	...
	public function delete(key:K, comparator:Ord<K>):Map<K, V>
		return switch this {
			case Tip:
				Tip;
	...
	public function insert(kx:K, x:V, comparator:Ord<K>):Map<K, V>
		return switch this {
			case Tip:
				singleton(kx, x);
	...
	public function map<B>(f:V->B):Map<K, B>
		return switch this {
			case Tip: Tip;
	...
	public function values():Array<V>
		return foldLeft([], function(acc, v) {
			acc.push(v);
			return acc;
		});
	...
	function glue(that:Map<K, V>):Map<K, V>
	...
	static function deleteFindMin<K, V>(map:Map<K, V>)
		return switch map {
	...
	static function deleteFindMax<K, V>(map:Map<K, V>)
		return switch map {

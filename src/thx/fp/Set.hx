...
abstract Set<X>(SetImpl<X>) from SetImpl<X> to SetImpl<X> {
	...
	public function member(x:X, comparator:Ord<X>):Bool
		return switch this {
			case Tip: false;
	...
	public function insert(x:X, comparator:Ord<X>):Set<X>
		return switch this {
	...
	public function mapList<Y>(f:X->Y):List<Y>
		return switch this {
			case Tip:
				List.empty();
			case Bin(_, y, l, r):

...
abstract State<S, A>(S->Tuple<S, A>) from S->Tuple<S, A> {
	...
	inline static public function ap4<S, A, B, C, D, E>(f:A->B->C->D->E, s1:State<S, A>, s2:State<S, B>, s3:State<S, C>, s4:State<S, D>):State<S, E>
		return s4.ap(ap3(f.curry(), s1, s2, s3));

	inline static public function ap5<S, A, B, C, D, E, F>(f:A->B->C->D->E->F, s1:State<S, A>, s2:State<S, B>, s3:State<S, C>, s4:State<S, D>,
			s5:State<S, E>):State<S, F>
		return s5.ap(ap4(f.curry(), s1, s2, s3, s4));

	inline static public function ap6<S, A, B, C, D, E, F, G>(f:A->B->C->D->E->F->G, s1:State<S, A>, s2:State<S, B>, s3:State<S, C>, s4:State<S, D>,
			s5:State<S, E>, s6:State<S, F>):State<S, G>
		return s6.ap(ap5(f.curry(), s1, s2, s3, s4, s5));

	inline static public function ap7<S, A, B, C, D, E, F, G, H>(f:A->B->C->D->E->F->G->H, s1:State<S, A>, s2:State<S, B>, s3:State<S, C>, s4:State<S, D>,
			s5:State<S, E>, s6:State<S, F>, s7:State<S, G>):State<S, H>
		return s7.ap(ap6(f.curry(), s1, s2, s3, s4, s5, s6));

	inline static public function ap8<S, A, B, C, D, E, F, G, H, I>(f:A->B->C->D->E->F->G->H->I, s1:State<S, A>, s2:State<S, B>, s3:State<S, C>,
			s4:State<S, D>, s5:State<S, E>, s6:State<S, F>, s7:State<S, G>, s8:State<S, H>):State<S, I>

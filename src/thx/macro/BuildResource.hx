...
class BuildResource {
	...
	static function getResourceObject(cls:ClassType) {
		var o = {}, prefix = resolvePrefix(cls.meta);
	...
	static function getFromDir(dir:String, module:String, prefix:String) {
		...
		for (path in sys.FileSystem.readDirectory(dir)) {
			...
			var fullPath = '$dir/$path', name = normalizeName(path);
	...
	static function getMatchingFile(type:String, module:String, formats:Array<String>, prefix:String) {
		...
		var list = formats.map(function(format) return './$path.$format').filter(function(path) {
			...
			return (sys.FileSystem.exists(path) && !sys.FileSystem.isDirectory(path))
				|| (sys.FileSystem.exists(lc) && !sys.FileSystem.isDirectory(lc));
	...
	static function getContentFile(meta:MetaAccess, module:String, prefix:String) {
		return getFromFiles(formats.map(function(f) return {format: f, meta: ':$f'})
			.concat([{format: null, meta: ":file"}])
			.map(function(des) return !meta.has(des.meta) ? [] : meta.extract(des.meta)
				.map(function(v) return v.params)
				.flatten()
				.map(function(p) return ExprTools.getValue(p))
			...
			.flatten()
			.filter(function(item) return item != null), module, prefix);
	}

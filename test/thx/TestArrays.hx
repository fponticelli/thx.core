...
class TestArrays extends utest.Test {
	...
	public function testMapField() {
		var arr = [{a: 1}, {a: 2}, {a: 3}], test = arr.map(Functions.fn(_.a));
		Assert.same([1, 2, 3], test);
	}

	public function testMapFieldOnFunction() {
		var test = [new Sample(2), new Sample(3), new Sample(4)].map.fn(_.multiply(2));
		Assert.same([4, 6, 8], test);
	}

	public function testUsingMapField() {
		var arr = [{a: 1}, {a: 2}, {a: 3}], test = arr.map.fn(_.a);
		Assert.same([1, 2, 3], test);
	}

	public function testUsingMapFieldiOnFunction() {
		Assert.same([0, 3, 8], [new Sample(2), new Sample(3), new Sample(4)].mapi.fn(_.multiply(_1)));
	}
	...
	public function testContainsExact() {
		Assert.isTrue([1, 2, 3].containsExact(2));
		Assert.isFalse([1, 2, 3].containsExact(4));
	}

	public function testContainsWithPredicate() {
		Assert.isTrue([1, 2, 3].containsEq({v: 2}, function(e, i) return e == i.v));
		Assert.isFalse([1, 2, 3].containsEq({v: 4}, function(e, i) return e == i.v));
	}

	public function testContainsAll() {
		Assert.isTrue([1, 2, 3].containsAllExact([3, 1, 2]));
		Assert.isFalse([1, 2, 3].containsAllExact([3, 1, 2, 4]));
	}

	public function testContainsAny() {
		Assert.isTrue([1, 2, 3].containsAnyExact([2, 3, 4]));
		Assert.isFalse([1, 2, 3].containsAnyExact([4, 5, 6]));
	}

	public function testCount() {
		var arr = [2, 3, 2, 1, 4, 2, 3], map = arr.count();
	...
	public function testGroupByAnonymous() {
		...
		Assert.same([1 => [{level: 1}, {level: 1}], 2 => [{level: 2}],], results);
	}
	#end
	...
	public function testRotate() {
		Assert.same([["a0", "a1", "a2"], ["b0", "b1", "b2"]], Arrays.rotate([["a0", "b0"], ["a1", "b1"], ["a2", "b2"]]));
	}
	...
	public function testTraverseOption() {
		var arr = [1, 2, 3, 4, 5];
		var f = function(i:Int):Option<Int> {
			return if (i % 2 == 0) Some(i) else None;
		};
	...
	public function testTraverseValidation() {
		var arr = [1, 2, 3, 4, 5];
		var ff = function(i:Int):Validation.VNel<String, Int> {
			return if (i % 2 == 0) Validation.successNel(i) else Validation.failureNel('oops: $i');
		};
		var fs = function(i:Int):Validation.VNel<String, Int> {
			return Validation.successNel(i);
		};

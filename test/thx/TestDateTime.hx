...
class TestDateTime extends utest.Test {
	var offset:Time;
	var date:DateTime;
	...
	public function testLocalOffset() {
		var ref = DateHelper.localOffset(), delta = DateTime.localOffset();
		Assert.isTrue(ref == delta, 'expected $ref but got $delta');
	}
	#end

	public function testNow() {
		var ref = DateHelper.now(), date = DateTime.now();
	...
	function assertSnapTo(expected:DateTime, date:DateTime, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.isTrue(expected == t, 'expected $date to snap to $expected for $period but it is ${t.toString()}', pos);
	}

	function assertSnapPrev(expected:DateTime, date:DateTime, period:TimePeriod, ?pos:PosInfos) {
		var t = date.snapPrev(period);
		Assert.isTrue(expected == t, 'expected $date to snap before $expected for $period but it is ${t.toString()}', pos);
	}

	function assertSnapNext(expected:DateTime, date:DateTime, period:TimePeriod, ?pos:PosInfos) {
		var t = date.snapNext(period);
		Assert.isTrue(expected == t, 'expected $date to snap after $expected for $period but it is ${t.toString()}', pos);
	}
	...
	function assertParse(expected:DateTime, test:String) {
		switch DateTime.parse(test) {
			case Left(_):
				Assert.fail('unable to parse DateTime: $test');
			case Right(t):

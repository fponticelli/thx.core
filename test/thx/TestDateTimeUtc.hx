...
class TestDateTimeUtc extends utest.Test {
	...
	public function testFromToTime() {
		var d:Float = date.toTime(), date2:DateTimeUtc = d;
	...
	public function testNow() {
		var ref = DateHelper.nowUtc(), date = DateTimeUtc.now();
	...
	function assertParse(expected:DateTimeUtc, test:String) {
		switch DateTimeUtc.parse(test) {
			case Left(_):
				Assert.fail('unable to parse DateTimeUtc: $test');
			case Right(t):

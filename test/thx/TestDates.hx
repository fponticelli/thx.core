...
class TestDates extends utest.Test {
	public function testCreate() {
		var expectations = [
			// normal
			{expected: new Date(2014, 11, 1, 0, 0, 0), test: Dates.create(2014, 11, 1)}, // month overflow
			{expected: new Date(2015, 3, 1, 0, 0, 0), test: Dates.create(2014, 15, 1)},
			{expected: new Date(2013, 11, 1, 0, 0, 0), test: Dates.create(2014, -1, 1)}, // day overflow

			{expected: new Date(2014, 2, 4, 0, 0, 0), test: Dates.create(2014, 1, 32)},
			{expected: new Date(2013, 11, 31, 0, 0, 0), test: Dates.create(2014, 0, 0)}, // hour overflow

			{expected: new Date(2014, 1, 2, 2, 0, 0), test: Dates.create(2014, 1, 1, 26)},
			{expected: new Date(2013, 11, 31, 23, 0, 0), test: Dates.create(2014, 0, 1, -1)}, // minute overflow

			{expected: new Date(2014, 1, 1, 1, 5, 0), test: Dates.create(2014, 1, 1, 0, 65)},
			{expected: new Date(2013, 11, 31, 23, 59, 0), test: Dates.create(2014, 0, 1, 0, -1)}, // second overflow
	...
	function assertSnapTo(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.floatEquals(Date.fromString(expected).getTime(), t.getTime(), 'expected $date to snap to $expected for $period but it is ${t.toString()}', pos);
	}
	...
	function assertSnapPrev(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.floatEquals(Date.fromString(expected).getTime(), t.getTime(), 'expected $date to snap before $expected for $period but it is ${t.toString()}',
			pos);
	}
	...
	function assertSnapNext(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.floatEquals(Date.fromString(expected).getTime(), t.getTime(), 'expected $date to snap after $expected for $period but it is ${t.toString()}',
			pos);
	}
}

...
class TestEithers extends utest.Test {
	...
	public function testLeftMap() {
		...
		Assert.same(l2, l.leftMap(function(i) {
			return i + 1;
		}));
		Assert.same(rt, rt.leftMap(function(i) {
			return i + 1;
		}));
	}
	...
	public function testGetOrElseF():Void {
		...
		Assert.same("x", Right("x").getOrElseF(() -> {
			Assert.fail();
			return "y";
		}));
	}
	...
	public function testOrElseF():Void {
		...
		Assert.same(Right(1), Right(1).orElseF(() -> {
			Assert.fail();
			return Left("x");
		}));
		Assert.same(Right(1), Right(1).orElseF(() -> {

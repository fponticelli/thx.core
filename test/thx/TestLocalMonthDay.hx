...
class TestLocalMonthDay extends utest.Test {
	...
	public function testDayMonth() {
		...
		expectations.each(function(o) {
			...
			Assert.isTrue(o.day == d.day && o.month == d.month,
				'expected day ${o.day} == ${d.day} and month ${o.month} == ${d.month} for month/day ${o.test}');
		});
	}
	...
	public function testAdd() {
		var d = first.addDays(2).addMonths(9), e = "--10-03";
	...
	function assertSnapTo(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.isTrue(LocalMonthDay.fromString(expected) == t, 'expected $date to snap to $expected for $period but it is ${t.toString()}', pos);
	}
	...
	function assertSnapPrev(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.isTrue(LocalMonthDay.fromString(expected) == t, 'expected $date to snap before $expected for $period but it is ${t.toString()}', pos);
	}
	...
	function assertSnapNext(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.isTrue(LocalMonthDay.fromString(expected) == t, 'expected $date to snap after $expected for $period but it is ${t.toString()}', pos);
	}
}

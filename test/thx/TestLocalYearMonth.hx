...
class TestLocalYearMonth extends utest.Test {
	...
	public function testAdd() {
		var d = date.addYears(2).addMonths(9), e = "2018-04";
	...
	function assertSnapTo(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.isTrue(LocalYearMonth.fromString(expected) == t, 'expected $date to snap to $expected for $period but it is ${t.toString()}', pos);
	}
	...
	function assertSnapPrev(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.isTrue(LocalYearMonth.fromString(expected) == t, 'expected $date to snap before $expected for $period but it is ${t.toString()}', pos);
	}
	...
	function assertSnapNext(expected:String, date:String, period:TimePeriod, ?pos:PosInfos) {
		...
		Assert.isTrue(LocalYearMonth.fromString(expected) == t, 'expected $date to snap after $expected for $period but it is ${t.toString()}', pos);
	}
}

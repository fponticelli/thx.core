...
class TestOptions extends utest.Test {
	...
	public function testSpread() {
		var result = Options.combine2(Some("a"), Some("b")).spread2(function(a, b) {
			return a + b;
		});
	...
	public function testCataf() {
		Assert.same(0, None.cataf(() -> 0, v -> {
			Assert.fail();
			return v - 1;
		}));
		Assert.same(9, Some(10).cataf(() -> {
			Assert.fail();
			return 0;
		}, v -> v - 1));
	}
	...
	public function testGetOrElseF() {
		...
		Assert.same(10, Some(10).getOrElseF(() -> {
			Assert.fail();
			return 20;
		}));
	}
	...
	public function testOrElseF() {
		...
		Assert.same(Some(10), Some(10).orElseF(() -> {
			Assert.fail();
			return None;
		}));
		Assert.same(Some(10), Some(10).orElseF(() -> {

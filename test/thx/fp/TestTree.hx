...
@:access(thx.fp) class TestTree extends utest.Test {
	function construct():KTree<String> {
		return Branch('root', [
			Branch('l', [Branch('ll'), Branch("lr")]),
			Branch('m', [Branch('ml'), Branch('mm'), Branch('mr')]),
			Branch('r')
		]);
	}
	...
	public function testZipEdit() {
		...
		var folded = leaf.foldLeft(List.empty(), function(memo, next) {
